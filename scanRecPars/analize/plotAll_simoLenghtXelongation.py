...
def find_bins(run,  expr, cut):
    ...
    _nbinsqt =int(((15 - min(vals)) / deltabin)+ 1)
    ...
    binning = numpy.linspace(min(vals), 15, nbinsqt)
...
class plotAll_simo(ixpeDqmTask):
    ...
    def do_run(self, **kwargs):
        ...
        for i in range(0,len(my_bins)-1):
            ...
            outFolder=kwargs.get('output_folder')+'/LxELbin_'+str(i)+'/'
            ...
            nBinsLW =int(((15 - 0) / 0.01) + 1)                        
            LW_binning=numpy.linspace(0, 15, nBinsLW)            
            histLW = ixpeHistogram1d(LW_binning, xtitle="LxE")
            LW = self.run_list.values(expr, cut) #!!!! qua fa il lavoro!!!
            histLW.fill(LW)
            self.add_plot('LxE_'+str(i), histLW, figure_name='LxE_'+str(i), stat_box_position='upper right',  save=False)
            self.save_figure('LxE_'+str(i), overwrite=True)
            plt.savefig(outFolder+'LxE_'+str(i)+'.png')
            LW_mean=histLW.mean[0]
            LW_rms=histLW.rms[0]
            print("mean LxE=",histLW.mean[0], "rms=",histLW.rms[0])                        
            ...
            nomefileout= outFolder+'prova_outLxE.txt'
