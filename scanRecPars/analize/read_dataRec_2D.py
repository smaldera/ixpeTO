...
dirs=['001461','001436', '001416', '001471','001361', '001333']
...
def plot_all(baseRec1,outdir,folder):
   ...
   # y_edges=np.array(range(14,39,2) )
    x_edges= np.linspace(12, 30,10)
    y_edges= np.linspace(12,40,15)


    x= baseRec1.dict_rec['zero_thr']
    y=baseRec1.dict_rec['moma1_thr']
    z=baseRec1.dict_rec['modulation2']

    mod1=np.array(z)
    index_max_mod1= np.where(mod1==np.amax(mod1))[0][0]
   
    print ('max z = ',np.amax(mod1))
    print ('INDEX MAX= ',index_max_mod1)
    print ('x_max=',x[index_max_mod1])
    print ('y_max=',y[index_max_mod1])
    print("x_edges=",x_edges)
    print ("x=",x)
    
    hist_map = ixpeHistogram2d(x_edges, y_edges,  xtitle='ZERO THR', ytitle='MOMA 12 THR')
    #hist_map.fill(x, y,z)
    ...
    hist_map.fill(x, y,weights=z)
    hist_map.plot(cmin=1e-10)
    ax.plot(x[index_max_mod1], y[index_max_mod1], marker='o', markersize=10, color='red', label='max mod1') 
