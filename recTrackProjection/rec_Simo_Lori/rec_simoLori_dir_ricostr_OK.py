...
#        self.fit_spine()
    ...
    def rec_simo(self, coordinate_system, color_map='Reds', hexcol_padding=0.1,  text=True, show=True):
        ...
        #ruoto il SR di phi1 (quello che ha fatto SIMO): asse direzione ricostruita = asse x
        #ARRAY di COORDINATE
        xp = numpy.cos(phi1)*dx + numpy.sin(phi1)*dy    #x nel nuovo SR con asse ricostruito = asse x
        yp = -numpy.sin(phi1)*dx + numpy.cos(phi1)*dy   #y nel nuovo SR

        #angoli phi0 e phi1 nel nuovo SR
        phi0_bary=self.phi0-phi1      #angolo dell'asse principale
        phi1_bary=self.phi1-phi1      #angolo della direzione ricostruita.  sempre 0!!!!!!!!
        ...
        for i in range (0,len(self.cluster.x)):
             ...
             self.h1.SetTitle("Proiezione lungo asse ricostruito")
        ...
        #SR con DIREZ. RICOSTRUITA == ASSEX (come fatto da SIMO)
        ...
        #ELLIPS of INERTIA
        self.ellipse = ROOT.TEllipse(x_bary1,y_bary1,math.sqrt(self.mom2_long), math.sqrt(self.mom2_trans), 0,360, self.phi0*ROOT.TMath.RadToDeg()+self.phi1*ROOT.TMath.RadToDeg())
        ...
        self.distConv=0.-minX #in questo sitema di rif. il punto di conv e' in (0,0)         
        ...
        #self.newPointP= self.cerca_piccoAugerElectron(self.h1P_smoothSimo, fLin, minX,par) #????? DA FARE (rifare anhe cerca_picco.....P
        
        self.newPoint= self.cerca_piccoAugerElectron(self.h1_smoothSimo, minX)		#modified: no need to project on a function
    ...
    def cerca_piccoAugerElectron(self, h, minX):
       ...
       # restituisce le coord del picco auger 
        ...
        if (self.peakFinding==2): 
            ...
            #valore = raw_input('continueline682?')  
            self.fFit_histo=self.fitGaus_cutoff(h)
            #valore = raw_input('continueline684???????')  
        ...
        y_lin= 0.0     	#?????????????????????????????????????????????????????????????????????????????????????????????????????????????????
			#che y attribuisco? quella della retta della direzione ricostruita? o quella di una spline valutata in quel punto?
                       	#per ora gli ho dato la y presa dalla retta della direzione ricostruita, che qui e' l'asse x.	
                       	#y_bary2 e' la t del punto di conversione in questo SR
